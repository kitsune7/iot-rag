...
def iot_blueprint_generator(user_request: str) -> str:
    """
    Generate an initial IoT component blueprint based on user requirements.
    
    This tool produces a starting component set by analyzing keywords in the user's request
    and matching them to appropriate IoT components. Use this tool to create a basic 
    component list before conducting research for more detailed specifications.
    ...
    bullet_points = [f"- {comp_type.replace('_', ' ').title()}: {label}" for comp_type, label in components.items()]
    return "\n".join(bullet_points)